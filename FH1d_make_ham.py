...
state_dict, [hop_mat] = b.make_matrices_and_states(initial_state, DIMENSION, b.hop_nn_ob)

import pickle
pickle.dump( state_dict, open( "s16_FH_nn_statedict.p", "wb" ) )
b.save_sparse_csr("l16_FH_nn_hop",hop_mat)

int_mat = b.make_diag_ops(state_dict, lambda x: np.dot(x['up'],x['down']))

b.save_sparse_csr("l16_FH_nn_int",int_mat)
